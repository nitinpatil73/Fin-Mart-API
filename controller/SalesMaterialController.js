...
var GetSalesMaterial = function(req, res, next){
		...
		if(data!=null)
		{
			for (var i = 0 ; i < data[0].length; i++) {
				data[0][i].Product_image = "http://" + req.headers.host + "/uploads/salesmaterial/"+ data[0][i].Product_image;
			}
...
var GetSalesMaterialDocs = function(req, res, next){
		...
		if(data!=null)
		{
			var company = data[0];
			var docs = data[1];
			var hostpath = "http://bo.magicfinmart.com/";
			if(process.env.NODE_ENV == 'development'){
			    hostpath ="http://bo.mgfm.in/";
			}
			for (var i = 0; i < docs.length; i++) {				
			    	docs[i].image_path =hostpath + docs[i].image_path;
			}
