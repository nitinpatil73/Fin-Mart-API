...
var store_path='./public/OfflineDoc';
class OfflineQuotesDocumentController{};
String.prototype.replaceAll = function(target, replacement) {
  return this.split(target).join(replacement);
};

OfflineQuotesDocumentController.save=function(req,res){
  ...
  upload(req, res, function (err) {
      ...
      quoteid=quoteid.replaceAll('"',"");
      var filename=(req.file.filename).replaceAll('"',"");
      var destination = store_path+"/"+quoteid+"/"+filename;
      ...
      var DocType=(req.body.DocType).replaceAll('"',"")
       move(source,quoteid,DocName,function(err){
         if(err)
           base.send_response("Upload failed",null,res);
         else
           {
              var extension = source.split(".");
              newFilePath="OfflineDoc/"+quoteid+"/"+DocName+"."+extension[extension.length-1];
              var doc_param=[];
              doc_param.push(quoteid);
              doc_param.push(DocType);
              doc_param.push(DocName);
              doc_param.push(newFilePath);
              con.execute_proc('call offline_quotes_upload_doc(?,?,?,?)',doc_param,function(data) {
                if(data!=null){
...
module.exports=OfflineQuotesDocumentController;
